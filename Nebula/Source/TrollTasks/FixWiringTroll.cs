...
{
    /// <summary>
    /// 接线任务恶搞 - 逐渐增多的电线（8→16→70根）
    /// </summary>
    internal static class FixWiringTroll
    {
        ...
        internal static class WireMinigamePatch
        {
            ...
            private static void BeginPrefix(WireMinigame __instance)
            {
                ...
                TrollTaskManager.DestroyWireNodes(ParentLeftNode);
                float positionY = 2.25f;
                for (int i = 0; i < NumWires; i++)
                {
                    GameObject newGameObject = TrollTaskManager.BuildWireNode(
                        ParentLeftNode.FindChild("LeftWireNode").gameObject, ref positionY, NumWires);
                ...
                TrollTaskManager.DestroyWireNodes(ParentRightNode);
                positionY = 2.25f;
                for (int i = 0; i < NumWires; i++)
                {
                    GameObject newGameObject = TrollTaskManager.BuildWireNode(
                        ParentRightNode.FindChild("RightWireNode").gameObject, ref positionY, NumWires);
